...
function melos_thinkup_child_frontscripts() {
	...
	wp_enqueue_style( 'melos-thinkup-style-light', get_stylesheet_directory_uri() . '/style.css', array( 'thinkup-style' ), wp_get_theme()->get('Version') );
}
...
function melos_thinkup_updateoption_child_settings() {
	...
	$name_options_child   = $name_theme_lower . '_thinkup_child_settings_light';
	...
	if ( $options_child_settings != 1 ) {
		...
		$options_free['thinkup_styles_skin']       = 'light';
...
function melos_thinkup_removeoption_child_settings() {
...
add_action('switch_theme', 'melos_thinkup_removeoption_child_settings');
